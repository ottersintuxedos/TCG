...
; - also apply Magnifier, Defender, and other kinds of damage reduction accordingly
...
.non_zero_damage
	...
	jr .check_magnifier_and_defender
.safe
	call HandleDoubleDamageSubstatus
	ld a, e
	or d
...
.not_weak
	...
	call SwapTurn
	and b
	jr z, .check_magnifier_and_defender ; jump if not resistant
	ld hl, -30
	add hl, de
	ld e, l
	ld d, h
	ld hl, wDamageEffectiveness
	set RESISTANCE, [hl]
.check_magnifier_and_defender
	ld b, CARD_LOCATION_ARENA
	call ApplyAttachedMagnifier
	call SwapTurn
	ld b, CARD_LOCATION_ARENA
...
; increases de by 10 points for each Magnifier found in location b
ApplyAttachedMagnifier:
	push de
	call GetTurnDuelistVariable
	ld de, MAGNIFIER
