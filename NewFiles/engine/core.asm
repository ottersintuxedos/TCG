...
.print_color_icon
	...
	; print number of attached Magnifier and Defender with respective icon, if any
	ld hl, wHUDEnergyAndHPBarsX
	ld a, [hli]
	add 6
	ld b, a
	ld c, [hl] ; wHUDEnergyAndHPBarsY
	inc c
	ld a, DUELVARS_ARENA_CARD_ATTACHED_MAGNIFIER
	call GetTurnDuelistVariable
	or a
	jr z, .check_defender
	ld a, SYM_MAGNIFIER
	call WriteByteToBGMap0
	inc b
	ld a, [hl] ; number of attached Magnifier
	add SYM_0
	call WriteByteToBGMap0
	dec b
.check_defender
...
; face down stage card, color symbol, status symbol (if any), magnifier/defender
...
; color symbol, status symbol (if any), magnifier/defender symbols (if any),
...
PrintPlayAreaCardInformation:
	; print name, level, color, stage, status, magnifier/defender
...
; print a turn holder's play area Pokemon card's name, level, face down stage card,
; color symbol, status symbol (if any), and magnifier/defender symbols (if any).
; input:
   ; wCurPlayAreaSlot: PLAY_AREA_* of the card to display the information of
   ; wCurPlayAreaY: Y coordinate of where to print the card's information
...
.skip_status
	; finally check whether to print the Magnifier and/or Defender symbols
	ld a, [wCurPlayAreaSlot]
	add DUELVARS_ARENA_CARD_ATTACHED_MAGNIFIER
	call GetTurnDuelistVariable
	or a
	jr z, .not_magnifier
	ld a, [wCurPlayAreaY]
...
.something_to_handle
	...
	ld a, [hl]
	or a
	jr z, .discard_magnifier
	; has status condition
	call HandlePoisonDamage
	jr c, .discard_magnifier
	call HandleSleepCheck
	ld a, [hl]
	and CNF_SLP_PRZ
	cp PARALYZED
	jr nz, .discard_magnifier
	; heal paralysis
...
; discard any MAGNIFIER attached to the turn holder's arena and/or bench Pokemon
DiscardAttachedPluspowers:
	ld a, DUELVARS_ARENA_CARD_ATTACHED_MAGNIFIER
	call GetTurnDuelistVariable
	ld e, MAX_PLAY_AREA_POKEMON
	xor a
.unattach_magnifier_loop
	ld [hli], a
	dec e
	jr nz, .unattach_magnifier_loop
	ld de, MAGNIFIER
