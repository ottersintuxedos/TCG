...
10Plus10Attack_DamageBoostEffect:
	ld hl, 10
	call LoadTxRam3
	ldtx de, DamageCheckIfHeadsPlusDamageText
	call TossCoin_BankB
...
OldRod_PlayerSelection:
	...
	ld a, 2
	call TossCoinATimes_BankB
	ret po ; returns if the parity of heads is odd (ie. only one heads)
...
; display deck card list screen
	...
	ldtx ChooseBasicEnergyCardText
	ldtx de, DuelistDeckText
	bank1call SetCardListHeaderText
...
; makes list in wDuelTempList with all Energy cards
; that are in Turn Duelist's hand.
; if list turns out empty, return carry.
CreateEnergyCardListFromHand: ; 2f8b6 (b:78b6)
	ld a, DUELVARS_NUMBER_OF_CARDS_IN_HAND
	call GetTurnDuelistVariable
	ld b, a
	add DUELVARS_DECK_CARDS
...
.loop_b_press
	...
	ld bc, MURKROW
	call CompareDEtoBC
	jr z, .play_sfx ; found Murkrow, go back to top loop
.next
	inc l
	ld a, l
	cp DECK_SIZE
